...
export function useCopyToClipboard({
	...
	// Mirrors React's useEffect(fn, [ticket, status, timeout]) — reset to idle
	// after `timeout` once a copy/error has been shown.
	effect(
		() => ({ t: ticket(), s: status(), to: timeout }),
		({ t, s, to }) => {
			if (t === 0 || s === "idle") return
			const id = setTimeout(() => setStatus("idle"), to)
